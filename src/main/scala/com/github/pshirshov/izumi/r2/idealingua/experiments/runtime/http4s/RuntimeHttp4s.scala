...
import com.github.pshirshov.izumi.r2.idealingua.experiments.runtime.circe.{IRTClientMarshallers, IRTServerMarshallers}
import com.github.pshirshov.izumi.r2.idealingua.experiments.runtime.{IRTServerMultiplexor, _}
import fs2.Stream
...
class RuntimeHttp4s[R[_] : IRTServiceResult : Monad] {
  ...
  (
    ...
    def requestDecoder(context: Ctx, m: experiments.runtime.IRTMethod): EntityDecoder[R, muxer.Input] =
    ...
    val svc = AuthedService[Ctx, R] {
      case request@GET -> Root / service / method as ctx =>
        val methodId = experiments.runtime.IRTMethod(IRTServiceId(service), IRTMethodId(method))
